...
//NOTE: pin numbers below are NOT the raw openEMSstim example order anymore --
//they were swapped from the original (5,4,A2)=ch1 / (6,7,A3)=ch2 example
//assignment after LED/멀티미터/사람 테스트로 실제 보드의 채널1 출력이 진짜로
//(6,7,A3) 배선에서 나온다는 게 확인됐기 때문. 보드를 새로 배선했거나 다른
//보드로 바꿨다면 이 확인을 다시 해야 한다 (hardware_config.h 참고).
#if ENABLE_BLUETOOTH_MODULE
...
void handleSimpleCommand(String message) {
   ...
   if (message == "STATUS") {
      ...
      Serial.print(CHANNEL_2_ENABLED ? "1" : "0");
      Serial.print(",ch2_active=");
      Serial.print(emsChannel2.isActivated() ? "1" : "0");
      Serial.print(",ch2_intensity=");
